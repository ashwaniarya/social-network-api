...
const SpaceUser = require('../../spaceuser/model')
...
const resolvers = {
  Query:{
    spaces: async (obj, args, context)=>{
      ...
      if(args.pageNumber){
        if(args.limit){
          spaces = await Space.find({ status:'active' })
            .sort({priority:-1})
      ...
      let json =  JSON.parse(JSON.stringify(spaces))
      return json      
    },
  ...
  Space:{
    post: async (obj,args) => {
      ...
      let posts = await Post.find({space:obj._id,status:'active'}).sort({priority:-1}).limit(5)
  ...
  Mutation:{
      ...
      try {
        ...
        if( space ) {
          ...
          if( savedSpaceUser ) return space
            ...
            throw new Error( "Internal Server Error : Unable to create space and user relation ship" )
          }
        }
        else {
    ...
    removeSpace: async (obj,args, context)=>{
      try{
        ...
        let spaceUser = await SpaceUser.find({ space: args.spaceId, user: user._id, type: "admin"})
    ...
    modifySpace: async (obj, args, context) => {
      ...
      console.log(args)
      let updatedCat = await Space.findByIdAndUpdate(args._id,{$set:{...args}},{new:true})
